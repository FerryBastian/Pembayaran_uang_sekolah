...
@section('breadcrumbs')
    <x-breadcrumb :items="[['label' => 'Siswa'], ['label' => 'Pembayaran']]" />
@endsection

@section('content')
    <div class="space-y-6">
        <div class="grid gap-4 md:grid-cols-3">
            <x-stat-card title="Total Transaksi" :value="number_format($jumlahTransaksi, 0, ',', '.')" description="Pembayaran milik Anda" color="primary" />
            ...
            <x-stat-card title="Kelas" :value="$siswa->kelas?->nama_kelas ?? '-'" description="NISN {{ $siswa->nisn }}" color="secondary" />
        </div>

        <x-card title="Riwayat Pembayaran" subtitle="Daftar transaksi pembayaran atas nama {{ $siswa->nama }}.">
            <form method="GET" action="{{ route('siswa.pembayaran.index') }}" x-ref="filterForm" x-data class="mb-4 grid gap-3 lg:grid-cols-[1fr_14rem_auto]">
                <div class="relative">
                    ...
                    <input
                        ...
                        placeholder="Cari order, tagihan, metode..."
                ...
                @if (request('search') || request('status'))
                    <a href="{{ route('siswa.pembayaran.index') }}" class="inline-flex items-center justify-center rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-primary hover:bg-blue-50">
                        Reset
                    </a>
                @endif
            </form>
